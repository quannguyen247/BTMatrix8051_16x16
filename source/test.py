...
ACK_CHAR   = b'K'
ERR_CHAR   = b'E'                  # [FIX] Firmware báo lỗi nhận data
...
def preview_matrix(path: str):
    """In preview 16×16 bằng ký tự màu ANSI terminal."""
    try:
...
def wait_for_ack(conn, label: str = ""):
    """
    Chờ ACK 'K' từ firmware.
    [FIX] Xử lý thêm byte 'E' — firmware báo lỗi nhận data (byte dropped).
    """
    ...
    while True:
        ...
        if byte == ACK_CHAR:
            ...
            raise ConnectionError("[FW ERROR] Firmware báo lỗi nhận data (byte dropped). Thử lại.")
        else:
            # Bỏ qua byte rác từ HC-05 (noise), tiếp tục chờ
            print(f"[ACK] Bỏ qua byte rác: 0x{byte.hex()}")
...
def main():
    ...
    try:
        ...
        print(f"\n[ERROR] {e}")
        print("[!] Firmware gặp lỗi — thử gửi lại frame...")
        try:
            wait_for_ack(conn, "retry ready")
            send_frame(conn, frame_data)
            wait_for_ack(conn, "retry display done")
            print("\n[OK]  Gửi lại thành công! 🎉\n")
        except Exception as retry_e:
            print(f"[ERROR] Gửi lại thất bại: {retry_e}")
